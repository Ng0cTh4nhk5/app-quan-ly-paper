...
public class ResearchTopicService {
    ...
    private static final String FB_BO_SUNG_HO_SO  = "BO_SUNG_HO_SO";
    private static final String FB_KET_QUA_PB     = "KET_QUA_PB";
    private static final String FB_HOP_DONG       = "HOP_DONG";
    private static final String FB_CHO_XU_LY      = "CHO_XU_LY";
    private static final String FB_DA_XU_LY       = "DA_XU_LY";
    ...
    private static final int    MIN_PHAN_BIEN_DE_XUAT = 2;
    ...
    public DeTaiResponse taoDeTai(TaoDeTaiRequest req, Long gvId) {
        ...
        requireRole(gv, ROLE_GIANG_VIEN);
    ...
    // 1b. CẬP NHẬT THÔNG TIN CƠ BẢN (DRAFT only)
    public DeTaiResponse capNhatDeTai(Long id, CapNhatDeTaiRequest req, Long gvId) {
        requireRole(loadUser(gvId), ROLE_GIANG_VIEN);
        DeTai dt = loadDeTai(id);
        requireOwner(dt, gvId);
        requireState(dt, TopicState.DRAFT);
        dt.setTenDeTai(req.getTenDeTai().trim());
        dt.setMoTa(req.getMoTa());
        dt.setLinhVuc(req.getLinhVuc());
        DeTai saved = deTaiRepository.save(dt);
        recordAudit(saved, "CAP_NHAT_DE_TAI", gvId, TopicState.DRAFT.name(), TopicState.DRAFT.name(),
    ...
    public PageDeTaiResponse danhSach(String trangThai, Pageable pg, CustomUserDetails cur) {
        ...
        Page<DeTai> page = ROLE_GIANG_VIEN.equals(cur.getRole())
                ? (statusFilter == null
    ...
    public DeTaiDetailResponse layChiTiet(Long id, CustomUserDetails cur) {
        DeTai dt = loadDeTai(id);
        if (ROLE_GIANG_VIEN.equals(cur.getRole()) && !dt.getChuNhiem().getId().equals(cur.getId()))
        ...
        // B5-1: Load hợp đồng + tổ phản biện data cho FE guard fields
        HopDong hd = hopDongRepository.findByDeTaiId(id).orElse(null);
        List<ThanhVienToPhanBien> tvList = thanhVienToPhanBienRepository
                .findByToPhanBien_DeTaiId(id);
        boolean hasOpenHdFeedback = feedbackRepository
                .findFirstByDeTaiIdAndLoaiAndTrangThaiOrderByCreatedAtDesc(id, FB_HOP_DONG, FB_CHO_XU_LY)
                .isPresent();

        return deTaiMapper.toDetailResponse(dt, tl, entries, hd, tvList, hasOpenHdFeedback);
    }
    ...
    public DeTaiResponse guiHoSo(Long id, Long gvId) {
        ...
        if (phanBienDeXuatRepository.countByDeTaiId(id) < MIN_PHAN_BIEN_DE_XUAT)
            throw new BusinessException("Vui lòng đề xuất ít nhất " + MIN_PHAN_BIEN_DE_XUAT + " phản biện.", HttpStatus.BAD_REQUEST);
        DeTai saved = fsmService.transition(id, TopicEvent.GV_GUI_HO_SO, gvId);
        notifyNckh("QLNCKH - Hồ sơ mới chờ xem xét", saved, "Có hồ sơ đề tài mới.");
        return deTaiMapper.toResponse(saved);
    }

    // 6. TIẾP NHẬN [FSM: PNCKH_TIEP_NHAN]
    public DeTaiResponse tiepNhan(Long id, Long nckhId) {
        requireRole(loadUser(nckhId), ROLE_NCKH);
        ...
        notifyOwner(saved, "QLNCKH - Hồ sơ đã được tiếp nhận", "Hồ sơ đề tài đã được tiếp nhận để sơ thẩm.");
        return deTaiMapper.toResponse(saved);
    }
    ...
    public DeTaiResponse yeuCauBoSung(Long id, YeuCauBoSungRequest req, Long nckhId) {
        ...
        fb.setDeTaiId(dt.getId()); fb.setLoai(FB_BO_SUNG_HO_SO); fb.setNoiDung(req.getNoiDung().trim());
        fb.setDeadlinePhanHoi(toEndOfDay(req.getDeadlinePhanHoi()));
        fb.setTrangThai(FB_CHO_XU_LY); fb.setNguoiTaoId(nckhId);
        feedbackRepository.save(fb);
        DeTai saved = fsmService.transition(id, TopicEvent.PNCKH_YEU_CAU_BO_SUNG, nckhId);
        notifyOwner(saved, "QLNCKH - Yêu cầu bổ sung hồ sơ", "Hạn: " + req.getDeadlinePhanHoi() + ".");
        return deTaiMapper.toResponse(saved);
    }
    ...
    public DeTaiResponse gvNopBoSung(Long id, Long gvId) {
        ...
        Feedback fb = loadOpenFeedback(id, FB_BO_SUNG_HO_SO);
        requireBeforeDeadline(fb.getDeadlinePhanHoi(), "Đã quá hạn nộp bổ sung.");
        fb.setTrangThai(FB_DA_XU_LY); feedbackRepository.save(fb);
        DeTai saved = fsmService.transition(id, TopicEvent.GV_NOP_BO_SUNG, gvId);
        notifyNckh("QLNCKH - GV đã nộp bổ sung", saved, "GV đã nộp bổ sung hồ sơ.");
        return deTaiMapper.toResponse(saved);
    }
    ...
    public DeTaiResponse tuChoiHoSo(Long id, TuChoiHoSoRequest req, Long nckhId) {
        ...
        notifyOwner(saved, "QLNCKH - Há»“ sÆ¡ bá»‹ tá»« chá»‘i",
    ...
    public DeTaiResponse lapToPhanBien(Long id, LapToPhanBienRequest req, Long nckhId) {
        ...
        if (members.stream().anyMatch(m -> !ROLE_TO_PHAN_BIEN.equals(m.getVaiTro())))
        ...
        members.forEach(m -> notifyUser(m, saved, "QLNCKH - Má»i pháº£n biá»‡n Ä‘á» tÃ i",
                "Tháº§y/cÃ´ Ä‘Æ°á»£c phÃ¢n cÃ´ng pháº£n biá»‡n. Háº¡n ná»™p: " + req.getDeadlineNop() + "."));
        notifyOwner(saved, "QLNCKH - Äá» tÃ i chuyá»ƒn sang pháº£n biá»‡n",
    ...
    public DeTaiResponse pbNopKetQua(Long id, KetQuaPBRequest req, Long pbId) {
        requireRole(loadUser(pbId), ROLE_TO_PHAN_BIEN);
        DeTai dt = loadDeTai(id);
        requireState(dt, TopicState.DANG_PHAN_BIEN);
        String kq = normalizeCode(req.getKetQua(), Set.of(KQ_CHAP_NHAN, KQ_CAN_SUA, KQ_TU_CHOI),
        ...
        notifyNckh("QLNCKH - ThÃ nh viÃªn PB Ä‘Ã£ ná»™p káº¿t quáº£", dt, "Má»™t thÃ nh viÃªn tá»• PB Ä‘Ã£ ná»™p káº¿t quáº£ Ä‘Ã¡nh giÃ¡.");
    ...
    public DeTaiResponse xetDuyetPB(Long id, XetDuyetPBRequest req, Long nckhId) {
        ...
        String qd = normalizeCode(req.getQuyetDinh(), Set.of(KQ_CHAP_NHAN, QD_YEU_CAU_SUA, KQ_TU_CHOI),
                "Quyáº¿t Ä‘á»‹nh xÃ©t duyá»‡t khÃ´ng há»£p lá»‡.");

        if (KQ_CHAP_NHAN.equals(qd)) {
            tpb.setKetQuaTongHop(KQ_CHAP_NHAN); toPhanBienRepository.save(tpb);
            DeTai saved = fsmService.transition(id, TopicEvent.PNCKH_ACCEPT_PB, nckhId);
            notifyOwner(saved, "QLNCKH - Káº¿t quáº£ PB Ä‘Æ°á»£c cháº¥p nháº­n",
                    "Káº¿t quáº£ pháº£n biá»‡n cháº¥p nháº­n. Äá» tÃ i chuyá»ƒn sang láº­p há»£p Ä‘á»“ng.");
            return deTaiMapper.toResponse(saved);
        }

        if (QD_YEU_CAU_SUA.equals(qd)) {
            ...
            Feedback fb = new Feedback();
            fb.setDeTaiId(dt.getId()); fb.setLoai(FB_KET_QUA_PB); fb.setNoiDung(nd);
            ...
            notifyOwner(saved, "QLNCKH - YÃªu cáº§u chá»‰nh sá»­a thuyáº¿t minh",
        ...
        DeTai saved = fsmService.transition(id, TopicEvent.PNCKH_TU_CHOI, nckhId);
        notifyOwner(saved, "QLNCKH - Äá» tÃ i bá»‹ tá»« chá»‘i sau PB", "Äá» tÃ i bá»‹ tá»« chá»‘i sau xÃ©t duyá»‡t káº¿t quáº£ PB.");
        return deTaiMapper.toResponse(saved);
    }
    ...
    public DeTaiResponse gvNopLaiThuyetMinh(Long id, Long gvId) {
        ...
        Feedback fb = loadOpenFeedback(id, FB_KET_QUA_PB);
        requireBeforeDeadline(fb.getDeadlinePhanHoi(), "ÄÃ£ quÃ¡ háº¡n ná»™p láº¡i thuyáº¿t minh.");
        if (!taiLieuRepository.existsByDeTaiIdAndLoaiFileAndUploadedAtAfter(id, LOAI_THUYET_MINH, fb.getCreatedAt()))
        ...
        notifyNckh("QLNCKH - GV Ä‘Ã£ ná»™p láº¡i thuyáº¿t minh", saved, "Giáº£ng viÃªn Ä‘Ã£ ná»™p láº¡i TM chá»‰nh sá»­a.");
        return deTaiMapper.toResponse(saved);
    }
    ...
    public DeTaiResponse soanHopDong(Long id, SoanHopDongRequest req, Long nckhId) {
        ...
        hd.setTyLeTamUng(req.getTyLeTamUng()); hd.setTrangThaiHopDong(HD_CHO_PHAN_HOI);
        ...
        notifyOwner(dt, "QLNCKH - CÃ³ dá»± tháº£o há»£p Ä‘á»“ng", "PhÃ²ng NCKH Ä‘Ã£ soáº¡n dá»± tháº£o há»£p Ä‘á»“ng, vui lÃ²ng xem xÃ©t.");
        return deTaiMapper.toResponse(dt);
    }
    ...
    public DeTaiResponse phanHoiHopDong(Long id, PhanHoiHopDongRequest req, Long gvId) {
        ...
        requireHopDongState(hd, HD_CHO_PHAN_HOI);
        ...
        if (Boolean.TRUE.equals(req.getDongY())) {
            ...
            notifyNckh("QLNCKH - GV Ä‘Ã£ Ä‘á»“ng Ã½ dá»± tháº£o HÄ", dt, "Giáº£ng viÃªn Ä‘á»“ng Ã½, cÃ³ thá»ƒ tiáº¿n hÃ nh kÃ½ HÄ.");
            return deTaiMapper.toResponse(dt);
        }

        String gc = requireText(req.getNoiDungGhiChu(), "Ná»™i dung gÃ³p Ã½ lÃ  báº¯t buá»™c khi khÃ´ng Ä‘á»“ng Ã½.");
        hd.setTrangThaiHopDong(HD_YEU_CAU_SUA); hopDongRepository.save(hd);
        Feedback fb = new Feedback();
        fb.setDeTaiId(dt.getId()); fb.setLoai(FB_HOP_DONG); fb.setNoiDung(gc);
        fb.setTrangThai(FB_CHO_XU_LY); fb.setNguoiTaoId(gvId);
        feedbackRepository.save(fb);
        recordAudit(dt, "GV_PHAN_HOI_HOP_DONG", gvId, TopicState.DANG_LAP_HOP_DONG.name(),
                TopicState.DANG_LAP_HOP_DONG.name(), gc);
        notifyNckh("QLNCKH - GV cÃ³ gÃ³p Ã½ dá»± tháº£o HÄ", dt, "GV cÃ³ gÃ³p Ã½: " + gc);
        return deTaiMapper.toResponse(dt);
    }
    ...
    public DeTaiResponse kyHopDong(Long id, MultipartFile fileScan, LocalDate ngayKy, Long nckhId) {
        ...
        notifyOwner(saved, "QLNCKH - Äá» tÃ i Ä‘Ã£ kÃ½ há»£p Ä‘á»“ng",
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 19. CAN ACTIONS — Danh sách hành động khả dụng (RT-01)
    // ═══════════════════════════════════════════════════════════════════════════
    @Transactional(readOnly = true)
    public Map<String, Boolean> getCanActions(Long deTaiId, CustomUserDetails cur) {
        DeTai dt = loadDeTai(deTaiId);
        String role = cur.getRole();
        Long userId = cur.getId();
        boolean isOwner = dt.getChuNhiem() != null && dt.getChuNhiem().getId().equals(userId);
        TopicState state = dt.getStatus();

        boolean hasTM = taiLieuRepository.existsByDeTaiIdAndLoaiFile(deTaiId, LOAI_THUYET_MINH);
        HopDong hd = hopDongRepository.findByDeTaiId(deTaiId).orElse(null);
        String hdStatus = hd != null ? hd.getTrangThaiHopDong() : null;
        boolean gvDaDongY = Set.of(HD_CHO_KY, HD_DA_KY).contains(hdStatus);
        boolean hasHdFeedback = feedbackRepository
                .findFirstByDeTaiIdAndLoaiAndTrangThaiOrderByCreatedAtDesc(deTaiId, FB_HOP_DONG, FB_CHO_XU_LY)
                .isPresent();

        // PB membership check for TO_PHAN_BIEN role
        boolean isPbMember = false;
        boolean pbChuaNop = false;
        boolean hasPbMembers = false;
        if (toPhanBienRepository.existsByDeTaiId(deTaiId)) {
            hasPbMembers = true;
            var tvOpt = thanhVienToPhanBienRepository
                    .findByToPhanBien_DeTaiIdAndNguoiDung_Id(deTaiId, userId);
            if (tvOpt.isPresent()) {
                isPbMember = true;
                pbChuaNop = KQ_CHUA_NOP.equals(tvOpt.get().getKetQua());
            }
        }

        Map<String, Boolean> actions = new LinkedHashMap<>();
        actions.put("guiHoSo", ROLE_GIANG_VIEN.equals(role) && isOwner
                && Set.of(TopicState.DRAFT, TopicState.CHO_CHINH_SUA_THUYET_MINH).contains(state) && hasTM);
        actions.put("boSungHoSo", ROLE_GIANG_VIEN.equals(role) && isOwner
                && state == TopicState.CHO_BO_SUNG_HO_SO);
        actions.put("xoaTaiLieu", ROLE_GIANG_VIEN.equals(role) && isOwner
                && state == TopicState.DRAFT);
        actions.put("xemHopDong", ROLE_GIANG_VIEN.equals(role) && isOwner
                && Set.of(TopicState.DANG_LAP_HOP_DONG, TopicState.DANG_THUC_HIEN).contains(state));
        actions.put("tiepNhan", ROLE_NCKH.equals(role)
                && state == TopicState.CHO_PNCKH_XEM_XET);
        actions.put("yeuCauBoSung", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_XEM_XET_BOI_PNCKH);
        actions.put("tuChoiHoSo", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_XEM_XET_BOI_PNCKH);
        actions.put("lapToPhanBien", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_XEM_XET_BOI_PNCKH);
        actions.put("nopKetQuaPB", ROLE_TO_PHAN_BIEN.equals(role)
                && state == TopicState.DANG_PHAN_BIEN && isPbMember && pbChuaNop);
        actions.put("xetDuyetPB", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_PHAN_BIEN && hasPbMembers);
        actions.put("soanHopDong", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_LAP_HOP_DONG
                && (hdStatus == null || Set.of("CHUA_SOAN", "CAN_SUA", HD_YEU_CAU_SUA).contains(hdStatus)));
        actions.put("kyHopDong", ROLE_NCKH.equals(role)
                && state == TopicState.DANG_LAP_HOP_DONG
                && HD_CHO_KY.equals(hdStatus) && gvDaDongY && !hasHdFeedback);
        actions.put("dongYHopDong", ROLE_GIANG_VIEN.equals(role) && isOwner
                && state == TopicState.DANG_LAP_HOP_DONG
                && Set.of("CHO_GV_XEM", HD_CHO_PHAN_HOI).contains(hdStatus)
                && !gvDaDongY && !hasHdFeedback);
        actions.put("rutDeTai", ROLE_GIANG_VIEN.equals(role) && isOwner
                && Set.of(TopicState.DRAFT, TopicState.CHO_PNCKH_XEM_XET,
                    TopicState.DANG_XEM_XET_BOI_PNCKH, TopicState.CHO_BO_SUNG_HO_SO).contains(state));
    ...
    private Feedback loadOpenFeedback(Long deTaiId, String loai) {
        return feedbackRepository
                .findFirstByDeTaiIdAndLoaiAndTrangThaiOrderByCreatedAtDesc(deTaiId, loai, FB_CHO_XU_LY)
